...
*/
void md5string(char *input, int ilen, char output[33])
{
	unsigned char temp[16];
	md5((unsigned char*)input, ilen, temp);
	for (int i = 0; i < 16; i++)
	{
		//__android_log_print(ANDROID_LOG_ERROR, "NETPLAY", "md5string %x\n", temp[i]);
		char a = (temp[i] >> 4) & 0x0f;
		sprintf(output + i * 2, "%x", a);
		a = temp[i] & 0x0f;
		sprintf(output + i * 2 + 1, "%x", a);
	}
}
